...
def page_courses():
    ...
    if u["role"] in ["instructor", "admin"]:
        with st.expander("➕ Tambah Kursus Baru"):
            title = st.text_input("Judul Kursus", key="add_course_title")
            desc = st.text_area("Deskripsi Kursus", key="add_course_desc")
            if st.button("Simpan Kursus", key="save_course_btn"):
                if title:
    ...
    for c in courses:
        with st.container(border=True):
            ...
            if u["role"] in ["instructor", "admin"]:
                with col2:
                    if st.button("Edit", key=f"edit_{c['id']}"):
                        new_t = st.text_input("Judul Baru", value=c['title'], key=f"title_{c['id']}")
                        new_d = st.text_area("Deskripsi Baru", value=c.get('description', ''), key=f"desc_{c['id']}")
                        if st.button("Simpan Perubahan", key=f"save_{c['id']}"):
                            update_course(c['id'], new_t, new_d)
...
def page_assignments(cid):
    ...
    if u["role"] in ["instructor", "admin"]:
        with st.expander("➕ Tambah Tugas"):
            title = st.text_input("Judul Tugas", key=f"title_a_{cid}")
            desc = st.text_area("Deskripsi Tugas", key=f"desc_a_{cid}")
            due = st.date_input("Batas Waktu", key=f"due_a_{cid}")
            pts = st.number_input("Poin", 0, 100, 10, key=f"pts_a_{cid}")
            if st.button("Simpan Tugas", key=f"save_a_{cid}"):
                if title:
                    supabase.table("assignments").insert({
                        "course_id": cid,
                        "title": title,
...
def page_quizzes(cid):
    ...
    if u["role"] in ["instructor", "admin"]:
        with st.expander("➕ Tambah Kuis"):
            title = st.text_input("Judul Kuis", key=f"title_q_{cid}")
            desc = st.text_area("Deskripsi Kuis", key=f"desc_q_{cid}")
            tlim = st.number_input("Durasi (menit)", 0, 300, 0, key=f"time_q_{cid}")
            if st.button("Simpan Kuis", key=f"save_q_{cid}"):
                if title:
                    supabase.table("quizzes").insert({
                        "course_id": cid,
                        "title": title,
